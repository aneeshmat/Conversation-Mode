"""
Conversation Mode - Linux (AEC + VAD + ML Voice Classifier + Relative Smooth Ducking + Tk GUI)
...
- Tiny ML classifier decides "my voice vs echo/music"
- VAD is only trusted when classifier says "my voice"
"""
...
# ---- import your classifier hook ----
from classifier_inference import classifier_prob  # expects classifier_prob(cleaned_dev, ref_dev)
...
# ---------------------------
# 9) PROCESSING WORKER (AEC → HP → classifier → VAD → duck)
# ---------------------------
def processing_worker():
    ...
    while running_worker:
        ...
        # Basic energy metrics (optional, can be used for extra gating)
        ...
        # ---- ML classifier: my voice vs echo/music ----
        p_cls = classifier_prob(cleaned_dev, ref_dev)

        # If classifier says "not my voice", force VAD to 0
        raw_prob = 0.0
        ...
        # Optional: extra sanity clamp if ref is loud
        if ref_rms > 0.01 and last_vad_prob > 0.7 and p_cls < 0.9:
            last_vad_prob = 0.0

        # Duck only when VAD is high and mic clearly active
