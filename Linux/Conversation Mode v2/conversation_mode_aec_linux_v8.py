...
"""
Conversation Mode - Linux (AEC + VAD + ML Classifier + Ducking + Tk GUI)

This version integrates:
- Silero VAD
- Your custom ML classifier (18‑dim MFCC+spectral features)
- Your feature_extractor.py
- AEC (C shared object)
"""
...
import torch
from voice_model import VoiceClassifier
from feature_extractor import extract_features   # <-- your real extractor
...
def setup_ml_classifier():
    global ml_clf, ML_ENABLED
    try:
        ml_clf = VoiceClassifier(input_dim=18)
        state = torch.load("voice_classifier.pt", map_location="cpu")
        ml_clf.load_state_dict(state)
        ml_clf.eval()
        ML_ENABLED = True
        print("✅ VoiceClassifier loaded")
    except Exception as e:
        print(f"⚠️ VoiceClassifier load failed: {e}")
        ML_ENABLED = False
...
def _torch_vad_prob_16k(audio_16k: np.ndarray) -> float:
    ...
    if n >= 1024:
        ...
        with _torch.no_grad():
            for w in wins:
                t = _torch.from_numpy(w.astype(np.float32))
...
# ---------------------------
# 9) ML CLASSIFIER WRAPPER
# ---------------------------
def ml_prob_from_frames(cleaned_48k: np.ndarray,
                        ref_48k: np.ndarray) -> float:
    if not ML_ENABLED or ml_clf is None:
        return 0.0
    feats = extract_features(
        cleaned_frame_48k=cleaned_48k,
        ref_frame_48k=ref_48k,
        sr_48k=DEVICE_RATE
    )
    with torch.no_grad():
        t = torch.from_numpy(feats).float().unsqueeze(0)
        prob = float(ml_clf(t).item())
...
def processing_worker():
    ...
    while running_worker:
        ...
        if cleaned_16k.size >= 512 and not too_quiet and not ref_dominates and not echo_like:
            silero_prob = vad_prob_16k(cleaned_16k[-VAD_WINDOW_16K:])
            ml_p = ml_prob_from_frames(cleaned_dev, ref_dev)
            raw_prob = 0.5 * silero_prob + 0.5 * ml_p
...
class DuckingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AEC + VAD + ML Classifier (Linux)")
        ...
        status = (
            ...
            f"Block={FRAME_SIZE}  "
            f"Attack={VAD_ATTACK:.2f}  Gain={GAIN_AFTER_AEC}x"
        )
