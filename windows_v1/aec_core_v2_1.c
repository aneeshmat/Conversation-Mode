...
EXPORT void aec_process_buffer(AEC_State* state, float* speaker_buf, float* mic_buf, float* out_buf, int len, int delay) {
    float mu = 0.1f; // Balanced learning rate
    float eps = 0.01f; // Stability factor
    ...
    for (int j = 0; j < len; j++) {
        ...
        // 1. Predict Echo AND Calculate Energy (for normalization)
        float y = 0;
        float energy = eps; 
        ...
        // 3. Normalized Update (Prevents the 986.0 explosion)
        float step = (mu * e) / energy;
        for (int i = 0; i < FILTER_LEN; i++) {
            state->w[i] += step * state->x[(ref_idx - i + MAX_DELAY) & MASK];
        }
