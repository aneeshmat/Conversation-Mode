...
- Volume ducking is RELATIVE to the CURRENT volume at duck time.
  - Ducks to (current * DUCK_RATIO) with smooth fades
  - Restores to the pre-duck baseline ONLY if you didn't change volume while ducked
  - If you change volume while running, the next duck uses the new baseline automatically
- Volume control prefers pactl (PulseAudio/PipeWire), falls back to amixer.
...
# --- Relative ducking configuration ---
DUCK_RATIO = 0.5            # duck to 50% of CURRENT volume
DUCK_SECS = 2.5             # how long to stay ducked
FADE_STEPS = 6              # number of steps for smooth fade (up & down)
FADE_STEP_MS = 30           # ms between fade steps
VOLUME_RESTORE_TOL = 2      # +/- percent tolerance to consider "unchanged" while ducked
...
def _parse_pactl_volume(stdout: str) -> int:
    # Extract all channel percents and average them
...
def _amixer_get_volume() -> int:
    ...
    try:
        ...
        if out.returncode == 0:
            # Parse first percentage like "[50%]"
...
def smooth_set_volume(from_p: int, to_p: int, steps: int = FADE_STEPS, step_ms: int = FADE_STEP_MS):
    """Smoothly ramp system volume from from_p to to_p in 'steps' increments."""
...
class DuckingApp:
    def __init__(self, root):
        ...
        self.status_lbl = ttk.Label(
            root,
            text=f"MIC={MIC_DEVICE_ID}  REF={REF_DEVICE_ID}  I/O={DEVICE_RATE}Hz  VAD={SAMPLE_RATE}Hz  Block={FRAME_SIZE}  DUCK={int(DUCK_RATIO*100)}%"
        )
